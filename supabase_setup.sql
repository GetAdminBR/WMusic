...
-- separate table for user-submitted music, kept apart from admin-managed catalog
...
begin
    ...
    if not exists (
        select 1 from pg_policies where schemaname = 'public' and tablename = 'user_uploads' and policyname = 'user_uploads_public_all'
    ) then
        create policy user_uploads_public_all on public.user_uploads for all to anon, authenticated using (true) with check (true);
    end if;
